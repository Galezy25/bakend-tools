...
describe("File api tests", () => {
    const app = express();
    const fileApi = new FileApi(FileApi.path.resolve(__dirname, "filestest"), "key string");
    ...
    test('Download file', async () => {
        const downloadRoute = fileApi.getDownloadRoute(pathFile, nameOnDownload);
        const response = await request(app).get(downloadRoute)
            .buffer()
            .parse((res, callback) => {
    ...
    test('Try to download expired file token', async (done) => {
        const downloadRoute = fileApi.getDownloadRoute(pathFile, nameOnDownload, 2);
        await new Promise(r => { setTimeout(r, 3000) })
        request(app).get(downloadRoute).expect(403).end((err) => {
            if (err) done(err);
            else done();
        })
    })

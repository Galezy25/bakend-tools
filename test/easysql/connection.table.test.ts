...
import { Constraint, ForeignKey, PrimaryKey } from '../../src/easysql/constraint';
...
describe('Connection tests', () => {
    ...
    test('Secure query', async () => {
        ...
        expect(connection.secureQuery(queryTemplate, { name: 'Simple', num: 25 }))
            .toBe("this is a test named 'Simple' 25");
        expect(connection.secureQuery(queryTemplate, { name: 'CURRENT_TIMESTAMP', num: null }))
            .toBe("this is a test named CURRENT_TIMESTAMP NULL");
        expect(connection.secureQuery(queryTemplate, { name: true, num: NaN }))
            .toBe("this is a test named true NaN");
        expect(connection.secureQuery(queryTemplate, { name: 'SELECT * FROM users', num: undefined }))
            .toBe("this is a test named 'SELECT * FROM users' DEFAULT");
    })
    
    test('Create Test database', async () => {
        await expect(connection.query('CREATE DATABASE IF NOT EXISTS test')).resolves;
    ...
    test('Create table', async () => {
        ...
        let table = await connection.createTable(tableTestsName, ...columns, ...constraints);
    ...
    test('Find, Find One of a empty table', async () => {
        ...
        await expect(table.findOne({ id: 1 })).rejects.toMatchObject({ code: 404 });
    })
    ...
    test('Add, modify and drop column', async () => {
        ...
        await expect(table.addColumn(new Column('phone', {
            dataType: 'BIGINT'
        }))).resolves
        columns = await connection.query('SHOW COLUMNS FROM ' + tableTestsName);
        expect(columns.find(column => column.Field === 'phone')).toBeDefined();

        await expect(table.modifyColumn(new Column('phone', {
            dataType: ['VARCHAR', 17]
        })))
        columns = await connection.query('SHOW COLUMNS FROM ' + tableTestsName);
        expect(columns.find(column => column.Field === 'phone')?.Type).toBe('varchar(17)');

        await expect(table.dropColumn('phone'));
        columns = await connection.query('SHOW COLUMNS FROM ' + tableTestsName);
        expect(columns.find(column => column.Field === 'phone')).not.toBeDefined();
    })
    ...
    test('Create rows', async () => {
        ...
        await expect(table.create(toRegister, ['name', 'email', 'leader'])).resolves
    })
    ...
    test("Delete row", async () => {
        ...
        await expect(table.findOne({ id: 100 })).rejects.toMatchObject({ code: 404 });
    })
})
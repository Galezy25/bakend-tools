...
export default class Table {
    ...
    */
    private RE_join = new RegExp(/^(\S+_inner)|(\S+_left)|(\S+_right)|(\S+_full)$/);
    ...
    public addColumn(column_def: string  | Column) {
        return this._con.query(`ALTER TABLE ${this._name} ADD COLUMN ${column_def.toString()}`);
    }
    ...
    public modifyColumn(column_def: string | Column) {
        return this._con.query(`ALTER TABLE ${this._name} MODIFY COLUMN ${column_def.toString()}`);
    }
    ...
    public changeColumn(old_name: string, column_def: string | Column) {
        return this._con.query(`ALTER TABLE ${this._name} CHANGE COLUMN ${old_name} ${column_def.toString()}`);
    }
    ...
    public renameColumn(old_name: string, new_name: string) {
        return this._con.query(`ALTER TABLE ${this._name} RENAME COLUMN ${old_name} TO ${new_name}`);
    }
    ...
    public dropColumn(column_name: string) {
        return this._con.query(`ALTER TABLE ${this._name} DROP COLUMN ${column_name}`);
    }
    ...
    public addConstraint(constraint_def: Constraint | string) {
        return this._con.query(`ALTER TABLE ${this._name} ADD ${constraint_def.toString()}`);
    }
    ...
    public dropConstraint(constraint_name: string) {
        return this._con.query(`ALTER TABLE ${this._name} DROP CONSTRAINT ${constraint_name}`);
    }
    ...
    public find<T = any[]>(ctx: {
        ...
        if (ctx._fields) { fields = ctx._fields.split(','); }

        let query = ' SELECT ' + fields.join(', ') + ' FROM ' + this._name + ' this ';
        ...
        if (joins.length > 0) {
            query += joins.map(join => {
                ...
                return (' ' + join.type + ' JOIN ' + join.table + ' ON ' + join.relation[0] + '=' + join.relation[1])
            }).join(' ')
        }
        ...
        if (ctx._sort !== undefined) {
            ...
            query += ' ORDER BY ' + auxSort[0] + ' ' + ((auxSort.length === 2) ? auxSort[1] : ' ASC ')
        }

        if (ctx._limit !== undefined || ctx._start !== undefined) {
            query += ' LIMIT ' + ((ctx._start) ? ctx._start + ', ' : '') +
     ...
     * }\>
     */
    public create(values: { [field: string]: any } | { [field: string]: any }[], fields: string[] = Object.getOwnPropertyNames((values instanceof Array) ? values[0] : values).filter(prop => (values instanceof Array) ? true : values[prop] !== undefined)) {
        let template = fields.map(field => '{{' + field + '}}').join(', ');
        let query = ' INSERT INTO ' + this._name + ' (' + fields.join(',') + ') VALUES ';
     ...
     * @returns Promise
     */
    public update(values: { [field: string]: any }, where_ctx: { [wherePattern: string]: any }, fields: string[] = Object.getOwnPropertyNames(values).filter(prop => values[prop] !== undefined)) {
        let query = ' UPDATE ' + this._name + ' SET ' +
            fields.map(field => (field + '={{' + field + '}}')).join(', ') + ' ' + this.getWhere(where_ctx);
    ...
    public delete(where_ctx: { [field: string]: any }) {
        let query = ' DELETE FROM ' + this._name + ' ' + this.getWhere(where_ctx);
    ...
    private getWhere(ctx: { [field: string]: any }) {
        ...
        propsWhere = propsWhere.filter(prop => {
            return prop !== '_sort' && prop !== '_limit' && prop !== '_start' && prop !== '_fields' && !this.RE_join.test(prop);
        })
        ...
        propsWhere = propsWhere.map(prop => {
            if (this.RE_n.test(prop)) {
                ...
                return prop.replace('_gteq', '') + ' >= ' + this.escape(ctx[prop]);
            } else if (this.RE_ls.test(prop)) {
                return prop.replace('_ls', '') + ' < ' + this.escape(ctx[prop]);
            } else if (this.RE_lseq.test(prop)) {
                return prop.replace('_lseq', '') + ' <= ' + this.escape(ctx[prop]);
            } else if (this.RE_like.test(prop)) {
                return prop.replace('_like', '') + ' LIKE ' + this.escape(ctx[prop]);
            } else if (this.RE_nin.test(prop)) {
                return prop.replace('_nin', '') + ' NOT IN (' +
                    ctx[prop].split(',').map((val: any) => this.escape(val)).join(',') + ')';
            } else if (this.RE_in.test(prop)) {
                return prop.replace('_in', '') + ' IN (' +
                    ctx[prop].split(',').map((val: any) => this.escape(val)).join(',') + ')';
            } else if (this.RE_btw.test(prop)) {
                let btwValues = ctx[prop].split(',');
                return prop.replace('_btw', '') + ' BETWEEN ' + this.escape(btwValues[0]) +
                    ' AND ' + this.escape(btwValues[1]);
            } else {
